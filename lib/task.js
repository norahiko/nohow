...
exports.addBuiltinTask = function() {
    ...
    commands.asyncTask('static-server', function(done) {
        server = commands.staticserver({ logging: true, listenCallback: done });
    });

    commands.asyncTask('close-static-server', function(done) {
        if(server) {
            server.close(done);
            server = null;
        }
