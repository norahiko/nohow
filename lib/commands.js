...
commands.env = null; // == jub.env -- initialized at jub.js
...
commands.expand = function expand(format /* ...args */) {
    ...
    return format.replace(expandExpression, function (exp) {
        ...
        var envVar;
        if(commands._reservedValue[varName]) {
            envVar = commands._reservedValue[varName]();
        ...
        for(i = 0; i < filters.length; i++) {
            envVar = commands._expandFilter[filters[i]](envVar);
        }
...
commands._extendsNativeObject = function() {
    if([].expandEach) {
        return;
    }
    ...
    Object.defineProperty(Array.prototype, 'expandEach', {
        ...
        value: function expandEach(exp) {
            return this.map(function(item) {
                return commands.expand(exp, item);
            });
        },
    });
...
commands.watch = function watch(pattern, tasks, func) {
    return new lazylib.Watcher(pattern, tasks, func);
};


commands.staticserver = function staticserver(options) {
...
commands.spawn = function spawn(file, args, options) {
    return lazylib.spawn.spawn(file, args, options);
