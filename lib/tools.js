...
function expandExpression(exp, args) {
    ...
    if(value === undefined) {
        throw new Error("jub.expand: 'env." + varName + "' is not defined");
    }
...
function getEnvValue(name, args) {
    if(tools._reservedValues[name]) {
        return tools._reservedValues[name]();
...
function expandAttributes(value, attrs, varName) {
    for(var i = 1; i < attrs.length; i++) {
        ...
        if(value === undefined) {
            ...
            throw new Error("jub.expand: 'env." + errorExpr + "' is not defined");
        }
    }
    return value;
}
...
function applyFilters(value, filters) {
    for(var i = 1; i < filters.length; i++) {
        value = tools._expandFilters[filters[i]](value);
    }
    return value;
}
