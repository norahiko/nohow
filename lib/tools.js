...
tools.trace = function trace(/* messages */) {
    ...
    var called = obj.stack.split("\n")[2];
    var calledInfo;
    if(called.indexOf("(") === -1) {
         calledInfo = called.match(/at (.*)/)[1].split(":");
    } else {
        calledInfo = called.match(/\((.*)\)$/)[1].split(":");
    }
    calledInfo[0] = pathModule.basename(calledInfo[0]);

    var args = ["(" + calledInfo.join(":") + ")"];
...
tools.Watcher = function Watcher(pattern, callback) {
...
tools.shell = function shell(command, options) {
    return lazylib.spawn.shell(command, options);
};


tools.system = function system(command, options) {
    return lazylib.spawn.system(command, options);
};
