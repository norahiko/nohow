...
tools.expand = function expand(format /* ...args */) {
    ...
    return format.replace(expandExpression, function (exp) {
        ...
        var envVar;
        if(tools._reservedValue[varName]) {
            envVar = tools._reservedValue[varName]();
        ...
        for(i = 0; i < filters.length; i++) {
            envVar = tools._expandFilter[filters[i]](envVar);
        }
...
tools.watch = function watch(pattern, tasks, func) {
...
tools.spawn = function spawn(file, args, options) {
    return lazylib.spawn.spawn(file, args, options);
    //var result = tools.spawnBuf(file, args, options);
