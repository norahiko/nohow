...
tools.listdir = function listdir(globpattern) {
    ...
    if(files.length === 0 && subfiles.length === 0) {
        throw new Error("nohow.listdir: '" + globpattern + "' no such file or directory");
    }
...
tools.popd = function popd() {
    ...
    if(dirpath === undefined) {
        throw new Error("nohow.popd: directory stack empty");
    }
    process.chdir(dirpath);
};
...
tools.move = function move(globpattern, destPath) {
    ...
    if(paths.length === 0) {
        throw new Error("nohow.move: '" + globpattern + "' no such file or directory");
...
tools.copy = function copy(globpattern, destPath) {
    ...
    if(srcPaths.length === 0) {
        throw new Error("nohow.copy: '" + globpattern + "' no such file or directory");
    }
    ...
    if(1 < srcPaths.length && destIsDir === false) {
        throw new Error("nohow.copy: '" + destPath + "' is not a directory");
    }
...
tools.replace = function replace(globpattern, from, to) {
    assert(from instanceof RegExp, "nohow.replace: arguments[1] must be a RegExp");
    assert(from.global, "nohow.replace: RegExp needs global flag (for example: /foo/g )");
...
tools.concat = function concat(globpattern, sep) {
    ...
    if(contents.length === 0) {
        throw new Error("nohow.concat: '" + globpattern + "' no such file or directory");
    }
