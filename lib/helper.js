...
// RuskJS uses lazy loaded libraries
// it makes startup fater
helper.lazylib = {};

setLazyLib(helper.lazylib, 'glob', 'glob');
setLazyLib(helper.lazylib, 'es6promise', 'es6-promise');
...
helper.findRuskfile = function() {
    var cwd = process.cwd();
    var files = fs.readdirSync(cwd);
    for(var i = 0; i < files.length; i++) {
        if(/^ruskfile/i.test(files[i])) {
            return pathModule.join(cwd, files[i]);
        }
    }
};
...
function searchLibs() {
    var libs = [];
    var libdir;
    var match = /^(.*\/node_modules)\/rusk.*/.exec(__filename);
    if(match) {
        libdir = match[1];
...
helper.copyDir = function(srcDir, destDir) {
    ...
    // copy files of srcDir
    var files = fs.readdirSync(srcDir);
    if(srcDir[srcDir.length - 1] !== pathModule.sep) {
