...
setLazyLib(helper.lazylib, "StaticServer", "./staticserver.js");
setLazyLib(helper.lazylib, "spawn", "./spawn.js");
...
helper.readContents = function(fd) {
    if(fd < 0) { return new Buffer(0); }
    var size = fs.fstatSync(fd).size;
    if(size === 0) { return new Buffer(0); }
    var result = new Buffer(size);
    var offset = 0;
    while(true) {
        var readSize = fs.readSync(fd, result, offset, size - offset);
        if(readSize === 0) {
            break;
        }
        offset += readSize;
