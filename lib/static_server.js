...
function normalizeOptions(options) {
    ...
    if(typeof options !== "object") {
        throw new TypeError(
            "Invalid arguments 'jub.StaticServer'\n" +
            "Usage:" +
            "  StaticServer({\n" +
            "    port         : 8000,         // optional\n" +
            "    host         : '',           // optional\n" +
    ...
    if(options.port) {
        ...
        result.port = Number(env.staticPort) || DEFAULT_PORT;
    }

    if(options.host) {
        result.host = tools.expand(options.host);
    } else {
        result.host = env.staticHost || DEAFAULT_HOST;
    }

    if(options.documentRoot) {
        result.documentRoot = tools.expand(options.documentRoot);
    } else {
        result.documentRoot = env.staticDocumentRoot || tools.env.root;
    }
